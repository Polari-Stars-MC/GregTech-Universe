...
import org.polaris2023.gtu.space.simulation.ksp.KspBodyDefinition;
import org.polaris2023.gtu.space.simulation.ksp.KspBodyState;
import org.polaris2023.gtu.space.simulation.ksp.KspReferenceFrameKind;
import org.polaris2023.gtu.space.simulation.ksp.KspSnapshot;
import org.polaris2023.gtu.space.simulation.ksp.KspVesselState;
...
public final class KspDebugWindow {
    ...
    private enum SidePage {
        SIMULATION(Component.translatable("gtu_space.ksp_debug.tab.simulation")),
        VESSEL(Component.translatable("gtu_space.ksp_debug.tab.vessel")),
        SELECTED_BODY(Component.translatable("gtu_space.ksp_debug.tab.selected_body")),
        LEGEND(Component.translatable("gtu_space.ksp_debug.tab.legend"));

        private final Component label;

        SidePage(Component label) {
            this.label = label;
        }
    }
    ...
    public static void render(GuiGraphics graphics) {
        ...
        drawScaledText(graphics, font, Component.translatable("gtu_space.ksp_debug.title"), panelX + 16, panelY + 10, COLOR_TEXT, HEADER_TITLE_SCALE);
        drawScaledText(graphics, font, Component.translatable("gtu_space.ksp_debug.hint"), panelX + 16, panelY + 21, COLOR_MUTED, HEADER_HINT_SCALE);
    ...
    private static KspSnapshot resolveSnapshot() {
        return KspClientSyncState.latestDebugSnapshot();
    }
    ...
    private static int renderSimulationPage(GuiGraphics graphics, Font font, KspSnapshot snapshot, int x, int y, int textWidth) {
        ...
        int lineY = y;
        drawScaledText(graphics, font, Component.translatable("gtu_space.ksp_debug.sim.tick", snapshot.simulationTick()), x, lineY, COLOR_MUTED);
        lineY += SIDE_LINE;
        drawScaledText(graphics, font, Component.translatable("gtu_space.ksp_debug.sim.time", String.format("%.2f", snapshot.simulationSeconds())), x, lineY, COLOR_MUTED);
        lineY += SIDE_LINE;
        drawScaledText(graphics, font, Component.translatable("gtu_space.ksp_debug.sim.universe", snapshot.universe().universeId()), x, lineY, COLOR_MUTED);
        lineY += SIDE_LINE;
        drawScaledText(graphics, font, Component.translatable("gtu_space.ksp_debug.sim.galaxy", snapshot.universe().galaxy().displayName()), x, lineY, COLOR_MUTED);
        lineY += SIDE_LINE;
        drawScaledText(graphics, font, Component.translatable("gtu_space.ksp_debug.sim.scale", snapshot.universe().scaleFactor().toPlainString()), x, lineY, COLOR_MUTED);
        lineY += SIDE_LINE;
        drawScaledText(graphics, font, Component.translatable("gtu_space.ksp_debug.sim.bodies", snapshot.bodies().size()), x, lineY, COLOR_MUTED);
        lineY += SIDE_LINE;
        drawScaledText(graphics, font, Component.translatable("gtu_space.ksp_debug.sim.vessels", snapshot.vessels().size()), x, lineY, COLOR_MUTED);
        lineY += SIDE_LINE;
        drawScaledText(graphics, font, Component.translatable("gtu_space.ksp_debug.sim.zoom", String.format("%.2f", zoom)), x, lineY, COLOR_MUTED);
        lineY += SIDE_LINE;
        drawScaledText(graphics, font, Component.translatable("gtu_space.ksp_debug.sim.body_scale", String.format("%.2f", bodyScale)), x, lineY, COLOR_MUTED);
        lineY += SIDE_LINE;
        drawScaledText(graphics, font, Component.translatable("gtu_space.ksp_debug.sim.pan", String.format("%.0f", panX), String.format("%.0f", panY)), x, lineY, COLOR_MUTED);
        lineY += SIDE_LINE;
        drawScaledText(graphics, font, Component.translatable("gtu_space.ksp_debug.sim.rot", String.format("%.0f", Math.toDegrees(cameraYaw)), String.format("%.0f", Math.toDegrees(cameraPitch))), x, lineY, COLOR_MUTED);
        lineY += SIDE_LINE;
        String trackName = highlightedBody == null ? Component.translatable("gtu_space.ksp_debug.sim.track_all").getString() : highlightedBody.definition().displayName();
        drawScaledText(graphics, font, Component.translatable("gtu_space.ksp_debug.sim.track", trackName), x, lineY, COLOR_MUTED);
        return lineY;
    }
    ...
    private static int renderVesselPage(GuiGraphics graphics, Font font, KspSnapshot snapshot, int x, int y, int textWidth) {
        ...
        if (vessel == null) {
            lineY += SIDE_TITLE;
            drawScaledText(graphics, font, Component.translatable("gtu_space.ksp_debug.vessel.none"), x, lineY, COLOR_MUTED);
        } else {
            lineY += SIDE_TITLE;
            ...
            lineY += SIDE_LINE;
            drawScaledText(graphics, font, Component.translatable("gtu_space.ksp_debug.vessel.primary", vessel.primaryBodyId()), x, lineY, COLOR_MUTED);
            lineY += SIDE_LINE;
            drawScaledText(graphics, font, Component.translatable("gtu_space.ksp_debug.vessel.speed", String.format("%.0f", vessel.absoluteVelocity().length())), x, lineY, COLOR_MUTED);
            ...
            if (primary != null) {
                ...
                lineY += SIDE_LINE;
                drawScaledText(graphics, font, Component.translatable("gtu_space.ksp_debug.vessel.alt", String.format("%.0f", altitude / 1000.0)), x, lineY, COLOR_MUTED);
            }
        }
        return lineY;
    }
    ...
    private static int renderSelectedBodyPage(GuiGraphics graphics, Font font, KspSnapshot snapshot, int x, int y, int textWidth) {
        ...
        if (highlightedBody == null) {
            drawScaledText(graphics, font, Component.translatable("gtu_space.ksp_debug.body.none"), x, y + SIDE_TITLE, COLOR_MUTED);
            return y + SIDE_TITLE;
        }
    ...
    private static int renderSelectedBodyStats(GuiGraphics graphics, Font font, KspBodyState body, KspSnapshot snapshot, int x, int y, int textWidth) {
        ...
        drawScaledText(graphics, font, Component.translatable("gtu_space.ksp_debug.body.speed", String.format("%.0f", body.absoluteVelocity().length())), x, y + SIDE_TITLE + SIDE_LINE, COLOR_MUTED);
        drawScaledText(graphics, font, Component.translatable("gtu_space.ksp_debug.body.history", snapshot.bodyHistory().getOrDefault(body.definition().id(), List.of()).size()), x, y + SIDE_TITLE + SIDE_LINE * 2, COLOR_MUTED);
        int nextLine = 3;
        if (body.referenceBodyId() != null) {
            ...
            if (parent != null) {
                ...
                drawScaledText(graphics, font, Component.translatable("gtu_space.ksp_debug.body.dist", String.format("%.0f", distance / 1000.0)), x, y + SIDE_TITLE + SIDE_LINE * nextLine, COLOR_MUTED);
                nextLine++;
                double roche = parent.definition().rocheLimit(body.definition());
                if (roche > 0.0) {
                    drawScaledText(graphics, font, Component.translatable("gtu_space.ksp_debug.body.roche", String.format("%.0f", roche / 1000.0)), x, y + SIDE_TITLE + SIDE_LINE * nextLine, COLOR_MUTED);
                    nextLine++;
                    if (distance < roche) {
                        drawScaledText(graphics, font, Component.translatable("gtu_space.ksp_debug.body.roche_violation"), x, y + SIDE_TITLE + SIDE_LINE * nextLine, 0xFFFF4444);
                        nextLine++;
                    }
                }
            }
        ...
        if (rotPeriod > 0) {
            ...
            drawScaledText(graphics, font, Component.translatable("gtu_space.ksp_debug.body.rot", hours, minutes), x, y + SIDE_TITLE + SIDE_LINE * nextLine, COLOR_MUTED);
            nextLine++;
            double tiltDeg = Math.toDegrees(body.definition().axialTiltRadians());
            drawScaledText(graphics, font, Component.translatable("gtu_space.ksp_debug.body.tilt", String.format("%.2f", tiltDeg)), x, y + SIDE_TITLE + SIDE_LINE * nextLine, COLOR_MUTED);
            nextLine++;
            double phaseDeg = Math.toDegrees(body.rotationPhaseRadians());
            drawScaledText(graphics, font, Component.translatable("gtu_space.ksp_debug.body.phase", String.format("%.1f", phaseDeg)), x, y + SIDE_TITLE + SIDE_LINE * nextLine, COLOR_MUTED);
            nextLine++;
        }
        if (body.perturbed()) {
            drawScaledText(graphics, font, Component.translatable("gtu_space.ksp_debug.body.perturbed"), x, y + SIDE_TITLE + SIDE_LINE * nextLine, 0xFFFF6644);
            nextLine++;
            drawScaledText(graphics, font, Component.translatable("gtu_space.ksp_debug.body.thrust", String.format("%.4f", body.thrustAcceleration().length())), x, y + SIDE_TITLE + SIDE_LINE * nextLine, COLOR_MUTED);
            nextLine++;
        }
    ...
    private static void renderLegend(GuiGraphics graphics, Font font, int x, int y, int width) {
        ...
        drawScaledText(graphics, font, Component.translatable("gtu_space.ksp_debug.legend.bodies"), x, lineY, COLOR_TEXT);
        lineY += SIDE_TITLE;
        legendRow(graphics, font, x, lineY, COLOR_ROOT, Component.translatable("gtu_space.ksp_debug.legend.sun"));
        legendRow(graphics, font, x, lineY + SIDE_LINE, COLOR_EARTH, Component.translatable("gtu_space.ksp_debug.legend.earth"));
        legendRow(graphics, font, x, lineY + SIDE_LINE * 2, COLOR_MOON, Component.translatable("gtu_space.ksp_debug.legend.moon"));
        legendRow(graphics, font, x, lineY + SIDE_LINE * 3, COLOR_VESSEL, Component.translatable("gtu_space.ksp_debug.legend.vessel"));

        lineY += SIDE_SECTION + SIDE_LINE * 4;
        drawScaledText(graphics, font, Component.translatable("gtu_space.ksp_debug.legend.orbits"), x, lineY, COLOR_TEXT);
        lineY += SIDE_TITLE;
        legendRow(graphics, font, x, lineY, COLOR_REFERENCE_TRACK, Component.translatable("gtu_space.ksp_debug.legend.guide_orbit"));
        legendRow(graphics, font, x, lineY + SIDE_LINE, COLOR_TRACK, Component.translatable("gtu_space.ksp_debug.legend.history_path"));
        legendRow(graphics, font, x, lineY + SIDE_LINE * 2, COLOR_SELECTED_TRACK, Component.translatable("gtu_space.ksp_debug.legend.selected_track"));
        legendRow(graphics, font, x, lineY + SIDE_LINE * 3, COLOR_VESSEL_ORBIT, Component.translatable("gtu_space.ksp_debug.legend.vessel_orbit"));
        legendRow(graphics, font, x, lineY + SIDE_LINE * 4, COLOR_PERIAPSIS, Component.translatable("gtu_space.ksp_debug.legend.periapsis"));
        legendRow(graphics, font, x, lineY + SIDE_LINE * 5, COLOR_APOAPSIS, Component.translatable("gtu_space.ksp_debug.legend.apoapsis"));

        lineY += SIDE_SECTION + SIDE_LINE * 6;
        drawScaledText(graphics, font, Component.translatable("gtu_space.ksp_debug.legend.projection"), x, lineY, COLOR_TEXT);
        lineY += SIDE_TITLE;
        drawScaledText(graphics, font, Component.translatable("gtu_space.ksp_debug.legend.proj_3d"), x, lineY, COLOR_MUTED);
        lineY += SIDE_LINE;
        drawScaledText(graphics, font, Component.translatable("gtu_space.ksp_debug.legend.proj_log"), x, lineY, COLOR_MUTED);
        lineY += SIDE_LINE;
        drawScaledText(graphics, font, Component.translatable("gtu_space.ksp_debug.legend.proj_track"), x, lineY, COLOR_MUTED);
        lineY += SIDE_LINE;
        drawScaledText(graphics, font, Component.translatable("gtu_space.ksp_debug.legend.proj_moon"), x, lineY, COLOR_MUTED);

        lineY += SIDE_SECTION + SIDE_LINE * 2;
        drawScaledText(graphics, font, Component.translatable("gtu_space.ksp_debug.legend.controls"), x, lineY, COLOR_TEXT);
        lineY += SIDE_TITLE;
        drawScaledText(graphics, font, Component.translatable("gtu_space.ksp_debug.legend.ctrl_pan"), x, lineY, COLOR_MUTED);
        drawScaledText(graphics, font, Component.translatable("gtu_space.ksp_debug.legend.ctrl_rotate"), x + columnWidth, lineY, COLOR_MUTED);
        lineY += SIDE_LINE;
        drawScaledText(graphics, font, Component.translatable("gtu_space.ksp_debug.legend.ctrl_zoom"), x, lineY, COLOR_MUTED);
        drawScaledText(graphics, font, Component.translatable("gtu_space.ksp_debug.legend.ctrl_cycle"), x + columnWidth, lineY, COLOR_MUTED);
        lineY += SIDE_LINE;
        drawScaledText(graphics, font, Component.translatable("gtu_space.ksp_debug.legend.ctrl_close"), x, lineY, COLOR_MUTED);
        drawScaledText(graphics, font, Component.translatable("gtu_space.ksp_debug.legend.ctrl_tabs"), x + columnWidth, lineY, COLOR_MUTED);

        lineY += SIDE_SECTION + SIDE_LINE;
        drawScaledText(graphics, font, Component.translatable("gtu_space.ksp_debug.legend.body_colors"), x, lineY, COLOR_TEXT);
        lineY += SIDE_TITLE;
        drawScaledText(graphics, font, Component.translatable("gtu_space.ksp_debug.legend.color_mercury"), x, lineY, COLOR_MUTED);
        lineY += SIDE_LINE;
        drawScaledText(graphics, font, Component.translatable("gtu_space.ksp_debug.legend.color_venus"), x, lineY, COLOR_MUTED);
        lineY += SIDE_LINE;
        drawScaledText(graphics, font, Component.translatable("gtu_space.ksp_debug.legend.color_earth"), x, lineY, COLOR_MUTED);
        lineY += SIDE_LINE;
        drawScaledText(graphics, font, Component.translatable("gtu_space.ksp_debug.legend.color_moon"), x, lineY, COLOR_MUTED);
        lineY += SIDE_LINE;
        drawScaledText(graphics, font, Component.translatable("gtu_space.ksp_debug.legend.color_outer"), x, lineY, COLOR_MUTED);

        lineY += SIDE_SECTION + SIDE_LINE * 4;
        drawScaledText(graphics, font, Component.translatable("gtu_space.ksp_debug.legend.track_meaning"), x, lineY, COLOR_TEXT);
        lineY += SIDE_TITLE;
        drawScaledText(graphics, font, Component.translatable("gtu_space.ksp_debug.legend.track_guide"), x, lineY, COLOR_MUTED);
        lineY += SIDE_LINE;
        drawScaledText(graphics, font, Component.translatable("gtu_space.ksp_debug.legend.track_history"), x, lineY, COLOR_MUTED);
        lineY += SIDE_LINE;
        drawScaledText(graphics, font, Component.translatable("gtu_space.ksp_debug.legend.track_selected"), x, lineY, COLOR_MUTED);
        lineY += SIDE_LINE;
        drawScaledText(graphics, font, Component.translatable("gtu_space.ksp_debug.legend.track_vessel"), x, lineY, COLOR_MUTED);
    }
    ...
    private static void legendRow(GuiGraphics graphics, Font font, int x, int y, int color, Component label) {
    ...
    private static void drawScaledText(GuiGraphics graphics, Font font, Component text, int x, int y, int color) {
        drawScaledText(graphics, font, text, x, y, color, SIDE_TEXT_SCALE);
    }
    ...
    private static void drawScaledText(GuiGraphics graphics, Font font, Component text, int x, int y, int color, float scale) {
        graphics.pose().pushPose();
        graphics.pose().translate(x, y, 0.0F);
        graphics.pose().scale(scale, scale, 1.0F);
        graphics.drawString(font, text, 0, 0, color, false);
